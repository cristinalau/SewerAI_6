...
macp_base AS (
    SELECT
        ...
        a.title                                            AS work_order_task_title,
        --s.assetnumber || ' - ' || s.title 			   AS work_order_task_title,
...
pacp_base AS (
    SELECT
        ...
        CASE
            ...
            WHEN e1.facilitytype = 10 AND ep2.unknfactype IN (4, 5) THEN 'PACP'
            ELSE NULL
        END                                                AS inspection_type,
        'Pipe'                                             AS pip_type,
        CASE
    ...
    WHERE
        (e1.facilitytype = 8
        OR (e1.facilitytype = 10 AND ep2.unknfactype IN (4, 5))      
        )
        AND NOT (
          s.assetnumber = '1195313'
        )
...
lacp_base AS (
    ...
    WHERE
        (e1.facilitytype = 4
        OR (e1.facilitytype = 10 AND ep2.unknfactype IN (6, 7, 8))        
        )
        AND NOT (
          s.assetnumber = '1195313'
        )
...
pacp_out AS (
    SELECT
        ...
        CAST(
            CASE
                /* DATA QUALITY CHECK (UPSTREAM)
                   If ground or invert is NULL or 0 ? return NULL */
                WHEN b.raw_usgroundelev_fl IS NULL 
                  OR b.raw_usinvertelev_fl IS NULL
                  OR b.raw_usgroundelev_fl = 0
                  OR b.raw_usinvertelev_fl = 0
                THEN NULL

                /* VALID CALCULATION: depth = ground - invert */
        ...
        CAST(
            CASE
                /* DATA QUALITY CHECK (DOWNSTREAM)
                   If ground or invert is NULL or 0 ? return NULL */
                WHEN b.raw_dsgroundelev_fl IS NULL 
                  OR b.raw_dsinvertelev_fl IS NULL
                  OR b.raw_dsgroundelev_fl = 0
                  OR b.raw_dsinvertelev_fl = 0
                THEN NULL
...
lacp_out AS (
    SELECT
        ...
        CAST(
            CASE
                WHEN b.facility_type = 10 AND b.unknown_type IN (6, 7, 8) THEN b.facility_id
                ELSE b.base_pipe_ref
            END
            AS VARCHAR2(200)
