/*==============================================================================
  ...
  Added logic:
    - When a brand-new WORKORDERTASK is inserted and there is currently NO row in
      CUSTOMERDATA.EPSEWERAI_WOT_STG for the same WORKORDER_UUID, insert the task
      into staging with FEED_STATUS = 'UPDATED' instead of 'NEW'.
    - Otherwise preserve the existing behavior:
        * matched row changed  -> FEED_STATUS = 'UPDATED'
        * unmatched task, but WO already staged -> FEED_STATUS = 'NEW'
==============================================================================*/
...
BEGIN
  ...
  WHERE a.assetoI     = :NEW.ASSET_OI
    AND a.assetnumber = '1195313'
    AND ROWNUM = 1;

EXCEPTION
  WHEN NO_DATA_FOUND THEN
    NULL; -- asset not 1195313 ? do nothing
END;
  ...
  USING (
    SELECT
      ...
      (select uuid from mnt.asset where assetoi = :NEW.ASSET_OI)  AS workorder_uuid,
      :NEW.UUID                                         AS task_uuid,
      :NEW.TASKNUMBER                                   AS tasknumber,
      --NVL(:NEW.WOTASKTITLE, wo.TITLE)                 AS wotasktitle,
      (select title from mnt.asset where assetoi = :NEW.ASSET_OI) AS wotasktitle,
      ...
      /*
        If no staging row exists yet for this WORKORDER_UUID,
        then this new task should be inserted as UPDATED.
      */
      CASE
        WHEN EXISTS (
          SELECT 1
          FROM CUSTOMERDATA.EPSEWERAI_WOT_STG sx
          --  WHERE sx.WORKORDER_UUID = wo.UUID
          WHERE sx.WORKORDER_UUID = (select uuid from mnt.asset where assetoi = :NEW.ASSET_OI)
        ) THEN 'NEW'
        ELSE 'UPDATED'
